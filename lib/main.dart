...
Future<void> main(List<String> args) async {
  ...
  // DESKTOP:
  //
  // O protocolo versin:// chega através de List<String> args.
  //
  // WEB:
  //
  // A URL atual fica disponível em Uri.base.
  //
  // O main.dart somente identifica e encaminha o URI.
  // O processamento da sessão de recovery permanece no
  // Supabase/AuthWrapper.
  //
  // ============================================================

  final initialDeepLink = _resolveInitialDeepLink(args);

  if (initialDeepLink != null) {
    debugPrint('[DEEP LINK] Inicial recebido: $initialDeepLink');
  }
...
// ============================================================
// RESOLVER DEEP LINK INICIAL
// ============================================================
//
// WEB:
//
// O callback pode chegar como:
//
...
//
// O Supabase Flutter processa a sessão Web durante sua própria
// inicialização. Mantemos Uri.base aqui para diagnóstico e para
// o AuthWrapper conhecer a URL inicial.
//
// DESKTOP:
//
// Procuramos o protocolo:
//
// versin://auth/reset-password
//
// recebido através do argumento %u do arquivo .desktop.
//
// ============================================================

Uri? _resolveInitialDeepLink(List<String> args) {
  // ==========================================================
  // WEB
  // ==========================================================
  ...
  if (kIsWeb) {
    ...
    final isPasswordRecoveryPath = uri.path == '/reset-password';
  ...
  for (final argument in args) {
    ...
    if (!normalizedArgument.startsWith('versin://')) {
      continue;
    }

    final uri = Uri.tryParse(normalizedArgument);
...
void _initializeAudioBackend() {
  ...
  JustAudioMediaKit.protocolWhitelist = const <String>['file', 'http', 'https'];
...
Future<void> _initializeMainApplication() async {
  ...
  await Supabase.initialize(
    ...
    authOptions: const FlutterAuthClientOptions(
      ...
      // ======================================================
      //
      // Mantemos implicit porque o fluxo atual de autenticação
      // do Versin já utiliza esse modo.
      //
      // No Web, supabase_flutter processa a URL de callback.
      //
      // No Desktop, o AuthWrapper processa o versin:// recebido
      // pelo main.dart através de getSessionFromUrl().
      //
      // ======================================================
...
Future<void> _configureStudioWindowChannels() async {
  ...
  await _mindMapChannel.setMethodCallHandler((call) async {
    switch (call.method) {
      ...
      case 'selectMindMapNode':
        ...
        studio.selectMindMapNode(data['node_id']?.toString());
  ...
  studio.addListener(() {
    unawaited(_pushStudioStateToExternalWindows(studio));
  });
...
Future<void> _pushMindMapToExternalWindow(StudioController studio) async {
  try {
    await _mindMapChannel.invokeMethod('setMindMap', {
      ...
      'nodes': studio.mindMapNodes.map((node) {
        return {
          'id': node.id,
