...
class _MatchPageState extends State<MatchPage> {
  ...
  void initState() {
    ...
    _matchSubscription = _matchController.matchEventStream.listen((projectId) {
      debugPrint(
        "🎯 MatchPage: Evento recebido no stream! Projeto: $projectId",
      );
      if (mounted) {
        navigateToNetworkingSession(projectId);
      }
    }, onError: (e) => debugPrint("❌ Erro no stream de match: $e"));
    ...
    // 3. Carrega os dados
    sl<MatchRepository>().streamCrossRoleMatches(
      _matchController,
      UserRole.artist,
    );
  }

  void navigateToNetworkingSession(String projectId) {
    debugPrint("🚀 Navegando para NetworkingSessionView com ID: $projectId");
    Navigator.push(
      context,
      MaterialPageRoute(
