...
class SongWordTimeline extends StatelessWidget {
  final List<String> words;

  final Color activeColor;

  final bool Function(String word) isWordUsed;
  ...
  int get usedWordsCount {
    return words.where(isWordUsed).length;
  }
  ...
  double get progress {
    ...
    return usedWordsCount / words.length;
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // =====================================================
          if (words.isEmpty)
            _EmptyTimeline(activeColor: activeColor, onAddWord: onAddWord)
          else
            _TimelineContent(
              words: words,
...
class _TimelineContentState extends State<_TimelineContent> {
  ...
  bool _scrollbarReady = false;

  @override
  void initState() {
    super.initState();

    _checkScrollPositionAfterFrame();
  }
  ...
  void didUpdateWidget(covariant _TimelineContent oldWidget) {
    ...
    if (oldWidget.words.length != widget.words.length || !_scrollbarReady) {
      _checkScrollPositionAfterFrame();
    }
  }

  void _checkScrollPositionAfterFrame() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (_scrollbarReady == ready) {
        return;
      }

      setState(() {
        _scrollbarReady = ready;
      });
    });
  }
  ...
  Widget build(BuildContext context) {
          ...
          children: [
            for (int index = 0; index < widget.words.length; index++) ...[
              _TimelineWordChip(
                ...
                used: widget.isWordUsed(widget.words[index]),
                activeColor: widget.activeColor,
                onRemove: () {
                  widget.onRemoveWord(widget.words[index]);
                },
              ),
              ...
              if (index < widget.words.length - 1)
                _TimelineConnector(
                  activeColor: widget.activeColor,
                  active:
                      widget.isWordUsed(widget.words[index]) &&
                      widget.isWordUsed(widget.words[index + 1]),
                ),
            ],
          ],
        ),
    ...
    if (!_scrollbarReady || !_scrollController.hasClients) {
      _checkScrollPositionAfterFrame();
...
class _EmptyTimeline extends StatelessWidget {
  ...
  const _EmptyTimeline({required this.activeColor, required this.onAddWord});
...
class _TimelineWordChip extends StatelessWidget {
  final String word;
  ...
  Widget build(BuildContext context) {
    return Tooltip(
      ...
      child: AnimatedContainer(
        ...
        constraints: const BoxConstraints(minWidth: 92, maxWidth: 175),
            ...
            Flexible(
              child: Text(
                ...
                style: TextStyle(
                  ...
                  fontSize: 12,
                  fontWeight: used ? FontWeight.w600 : FontWeight.w400,
                ),
              ),
            ),
...
class _TimelineConnector extends StatelessWidget {
  ...
  const _TimelineConnector({required this.activeColor, required this.active});
...
class _TimelineInfoBadge extends StatelessWidget {
  final IconData icon;
  ...
  @override
  Widget build(BuildContext context) {
    final resolvedColor = color ?? Colors.white38;
