...
class MatchQuickProjectResolver {
  static Future<MatchQuickProjectTarget?> resolve(String projectId) async {
    ...
    final project = await client.from('projects')
        .select('members,origin,status').eq('id', projectId).maybeSingle();
    if (project == null || project['origin'] != 'match' ||
    ...
    final profile = await client.from('profiles')
        .select('artist_name,username').eq('id', other).maybeSingle();
    final artist = profile?['artist_name']?.toString().trim() ?? '';
    final username = profile?['username']?.toString().trim() ?? '';
    return MatchQuickProjectTarget(other,
        artist.isNotEmpty ? artist : username.isNotEmpty ? username : 'Usuário');
  }
}
