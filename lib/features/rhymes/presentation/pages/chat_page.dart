...
class _ChatPageState
  ...
  void _initializeLogic() {
    _rhymesController.updateGamification(
      0.0,
    );
    _rhymesController.addListener(
  ...
  void _setupListeners() {
    _messageController.addListener(
      () {
        ...
        _rhymesController.onTextChanged(
          text,
        );
        if (text.isNotEmpty) {
          int lines = text
          ...
          _rhymesController.updateProgress(
            2,
            progress,
          );
        }
          ...
          (
              ...
              decoration: InputDecoration(
                ...
                enabledBorder: UnderlineInputBorder(
                  borderSide: BorderSide(
                    color: _rhymesController.getActiveColor(),
                  ),
                ),
              ),
            ),
            actions: [
              ...
              TextButton(
                onPressed: () {
                  setState(
                    () => _projectName = textController.text.toUpperCase(),
                  );
                  _autosaveProject();
                  Navigator.pop(
                    context,
  ...
  void _enviarEstruturaParaChat(
    List<
      String
    >
    blocos,
  ) {
    String textoEstrutura = blocos
        .map(
          (
            b,
          ) => "[$b]\n\n\n\n",
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            if (_rhymesController.isRhymeMode &&
                _rhymesController.suggestions.isNotEmpty)
              SuggestionBalloon(
                suggestion:
                    _rhymesController.suggestions[_currentSuggestionIndex %
                        _rhymesController.suggestions.length],
                onTap: () {
                  setState(
                    () {
                ...
                onAddCommand: () => _processMessage(
                  "Me dê um exemplo de rima com: ${_rhymesController.suggestions[_currentSuggestionIndex % _rhymesController.suggestions.length]}",
                ),
              ),
  ...
  @override
  void dispose() {
    _rhymesController.removeListener(
      _handleControllerChanges,
    );
