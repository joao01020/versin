// GENERATED REFACTOR NOTE:
// A API pública continua sendo TasksView.
// A implementação detalhada vive agora na camada page/, mantendo
// controller/repository/service contracts intactos.
//
// O arquivo ainda é grande por natureza da feature; a mudança
// evita alterar simultaneamente workflow, uploads, validação e UI.

import 'package:file_picker/file_picker.dart';
import 'package:flutter/material.dart';

import 'package:versin/modules/networking/tasks/controllers/project_tasks_controller.dart';
import 'package:versin/modules/networking/tasks/data/services/contribution_upload_service.dart';
import 'package:versin/modules/networking/tasks/models/contribution_delivery_model.dart';
import 'package:versin/modules/networking/tasks/models/project_contribution_model.dart';
import 'package:versin/modules/networking/tasks/models/project_task_member_model.dart';
import 'package:versin/modules/networking/tasks/widgets/production_flow_widget.dart';
import 'package:versin/modules/networking/tasks/widgets/project_milestones_widget.dart';
import 'package:versin/modules/networking/tasks/widgets/project_next_action_widget.dart';
import 'package:versin/modules/networking/tasks/widgets/project_record_widget.dart';
...
class _TasksPageViewState extends State<TasksPageView> {
  ...
  Future<void> _uploadDelivery(ProjectContributionModel contribution) async {
    ...
    try {
      ...
      _showMessage('Arquivo enviado e registrado na produção com sucesso.');
  ...
  // Fluxo:
  //
  // 1. baixa do bucket privado;
  // 2. valida SHA-256;
  // 3. abre o seletor de destino;
  // 4. grava os bytes pelo FilePicker;
  // 5. informa sucesso.
  //
  // O bucket continua privado e a policy SELECT do Storage
  // continua sendo a fonte de autorização.
  //
  // ============================================================
  ...
  Future<bool> _downloadDelivery(ContributionDeliveryModel delivery) async {
    ...
    if (expectedSha256.isEmpty) {
      _showMessage(
        'Não é possível baixar com segurança porque esta entrega não possui SHA-256 registrado.',
        error: true,
      );
    ...
    try {
      ...
      final result = await _uploadService.downloadVerified(
      ...
      // ========================================================
      //
      // O FilePicker recebe os bytes diretamente. Isso evita
      // acesso direto ao filesystem dentro desta View e mantém o
      // fluxo compatível com as plataformas suportadas pelo
      // plugin.
      //
      // ========================================================

      final savedPath = await FilePicker.platform.saveFile(
        dialogTitle: 'Salvar arquivo da contribuição',
